...
interface RelatorioProposta {
    ...
    notificacaoLida?: boolean; // Adicionado este campo para verificar o status da notificação
}
...
const RelatorioPropostas: React.FC = () => {
    ...
    const gerarPDF = async () => {
        ...
        const linhas = reportData.map((item) => {
            let displayStatus = item.statusProposta ?? 'Em Análise'; // Inicia com o status original ou 'Em Análise'
            if (item.notificacaoLida === true) { // Se a notificação for lida, atualiza o status
                displayStatus = 'Notificação Lida'; //
            }
            ...
            return {
                ...
                statusProposta: displayStatus, // Usa o status calculado
            };
        });
        ...
        autoTable(doc, {
            ...
            didDrawPage: (data: HookData) => {
                ...
                const imgX = 10;
                const imgY = 12;
                // Verificação para garantir que a logo não seja adicionada se logoBase64 for "valor"
                // ou outro valor inválido para uma imagem Base64.
        ...
        doc.setFontSize(12);
        doc.setTextColor('#333333');
        const finalY = doc.lastAutoTable?.finalY ?? doc.internal.pageSize.getHeight() - 30;
                ...
                <Box sx={{ width: '100%', mt: 3 }}>
                    ...
                    <TableContainer component={Paper} sx={{ maxHeight: 400 }}>
                        <Table stickyHeader>
                            ...
                            <TableBody>
                                {reportData.map((row) => {
                                    let displayStatus = row.statusProposta ?? 'Em Análise'; // Inicia com o status original ou 'Em Análise'
                                    if (row.notificacaoLida === true) { // Se a notificação for lida, atualiza o status
                                        displayStatus = 'Notificação Lida'; //
                                    }
                                    return (
                                        <TableRow key={row.idProposta}>
                                            ...
                                            <TableCell align="center">{displayStatus}</TableCell> {/* Usa o status calculado */}
                                        </TableRow>
                                    );
                                })}
                            </TableBody>

import React, { useState } from 'react';
import {
    Box,
    Typography,
    Button,
    TextField,
    Table,
    TableBody,
    TableCell,
    TableHead,
    TableRow,
    Paper,
    TableContainer,
    FormControl
} from '@mui/material';
import jsPDF from 'jspdf';
import { autoTable, HookData } from "jspdf-autotable";
import api from '../services/api';
import { useToast } from '../context/ToastContext';

// Tipagem dos dados do relatório de proposta
interface RelatorioProposta {
    idProposta: number;
    nomeProponente: string;
    idImovel: number;
    descricaoImovel: string;
    valorImovel: number;
    valorEntrada: number;
    rendaMensal: number;
    valorFinanciamento: number;
    prazo: number;
    dataProposta: string;
    statusProposta: string;
}

// Estende jsPDF para incluir propriedades do plugin autotable
interface jsPDFWithPlugin extends jsPDF {
    lastAutoTable?: {
        finalY: number;
    };
    internal: jsPDF['internal'] & {
        getNumberOfPages: () => number;
    };
}

// Retorna o mês/ano atual no formato AAAA-MM
const getCurrentMonthYear = () => {
    const now = new Date();
    const year = now.getFullYear();
    const month = String(now.getMonth() + 1).padStart(2, '0');
    return `${year}-${month}`;
};

const RelatorioPropostas: React.FC = () => {
    const [selectedMonth, setSelectedMonth] = useState<string>(getCurrentMonthYear());
    const [reportData, setReportData] = useState<RelatorioProposta[]>([]);
    const { showToast } = useToast();

    // Função para buscar os dados do relatório
    const fetchReportData = async () => {
        try {
            const response = await api.get<RelatorioProposta[]>(
                `/relatorios/propostas?mesAno=${selectedMonth}`
            );
            setReportData(response.data);

            if (response.data.length === 0) {
                showToast('Nenhum dado encontrado para o período solicitado.', 'info');
            } else {
                showToast('Dados do relatório de propostas carregados com sucesso!', 'success');
            }
        } catch (error) {
            console.error('Erro ao buscar dados do relatório de propostas:', error);
            showToast('Erro ao carregar dados do relatório de propostas. Tente novamente.', 'error');
            setReportData([]);
        }
    };

    // Função para gerar o PDF
    const gerarPDF = async () => {
        if (reportData.length === 0) {
            showToast('Não há dados para gerar o PDF.', 'info');
            return;
        }

        const doc = new jsPDF('landscape') as jsPDFWithPlugin;

        const [ano, mes] = selectedMonth.split('-');
        const mesesPt = [
            'Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho',
            'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'
        ];
        const titulo = `Relatório Mensal de Propostas de Compra - ${mesesPt[parseInt(mes, 10) - 1]} de ${ano}`;

        const generationDate = new Date().toLocaleDateString('pt-BR', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
            hour: '2-digit',
            minute: '2-digit',
        });

        const colunas = [
            { header: 'Proponente', dataKey: 'nomeProponente' },
            { header: 'ID Imóvel', dataKey: 'idImovel' },
            { header: 'Valor Imóvel (R$)', dataKey: 'valorImovel' },
            { header: 'Entrada (R$)', dataKey: 'valorEntrada' },
            { header: 'Renda Mensal (R$)', dataKey: 'rendaMensal' },
            { header: 'Financ. (R$)', dataKey: 'valorFinanciamento' },
            { header: 'Prazo (meses)', dataKey: 'prazo' },
            { header: 'Data Proposta', dataKey: 'dataProposta' },
            { header: 'Status', dataKey: 'statusProposta' },
        ];

        const linhas = reportData.map((item) => ({
            nomeProponente: item.nomeProponente ?? 'N/A',
            idImovel: item.idImovel?.toString() ?? 'N/A',
            valorImovel: item.valorImovel?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 2, maximumFractionDigits: 2 }) ?? 'R$ 0,00',
            valorEntrada: item.valorEntrada?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 2, maximumFractionDigits: 2 }) ?? 'R$ 0,00',
            rendaMensal: item.rendaMensal?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 2, maximumFractionDigits: 2 }) ?? 'R$ 0,00',
            valorFinanciamento: item.valorFinanciamento?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 2, maximumFractionDigits: 2 }) ?? 'R$ 0,00',
            prazo: item.prazo?.toString() ?? 'N/A',
            dataProposta: item.dataProposta ? new Date(item.dataProposta).toLocaleDateString('pt-BR') : 'N/A',
            statusProposta: item.statusProposta ?? 'N/A',
        }));

        const logoBase64 = "data:image/png;base64,";

        autoTable(doc, {
            head: [colunas.map((col) => col.header)],
            body: linhas.map((row) => Object.values(row)),
            startY: 40,
            theme: 'striped',
            styles: {
                font: 'helvetica',
                fontSize: 9, // Um pouco maior, pois temos mais espaço
                cellPadding: 1.8,
                valign: 'middle',
                overflow: 'linebreak'
            },
            headStyles: {
                fillColor: '#14453e',
                textColor: '#ffffff',
                fontStyle: 'bold',
                halign: 'center'
            },
            bodyStyles: {
                textColor: '#333333'
            },
            alternateRowStyles: {
                fillColor: '#f2f2f2'
            },
            // AJUSTE DAS LARGURAS DAS COLUNAS para 9 colunas em landscape, usando o espaço extra
            columnStyles: {
                0: { halign: 'left', cellWidth: 43 },  // Proponente (era 38, agora 43)
                1: { halign: 'right', cellWidth: 19 }, // ID Imóvel (era 18, agora 19)
                2: { halign: 'right', cellWidth: 32 }, // Valor Imóvel (era 28, agora 32)
                3: { halign: 'right', cellWidth: 32 }, // Entrada (era 28, agora 32)
                4: { halign: 'right', cellWidth: 32 }, // Renda Mensal (era 28, agora 32)
                5: { halign: 'right', cellWidth: 32 }, // Valor Financ. (era 28, agora 32)
                6: { halign: 'center', cellWidth: 22 }, // Prazo (novo, era 20, agora 22)
                7: { halign: 'center', cellWidth: 29 }, // Data Proposta (era 25, agora 29)
                8: { halign: 'center', cellWidth: 29 }  // Status (era 25, agora 29)
            },
            didDrawPage: (data: HookData) => {
                const pageWidth = doc.internal.pageSize.getWidth();
                // const pageHeight = doc.internal.pageSize.getHeight(); // Não usado diretamente, mas útil para referência

                if (logoBase64 && logoBase64 !== "data:image/png;base64,") {
                    const imgWidth = 22;
                    const imgHeight = 22;
                    const imgX = 10;
                    const imgY = 12;
                    doc.addImage(logoBase64, "PNG", imgX, imgY, imgWidth, imgHeight);
                }

                doc.setFontSize(16);
                doc.setTextColor('#333333');
                doc.text(titulo, pageWidth / 2, 26, { align: 'center' });

                doc.setFontSize(10);
                doc.setTextColor('#666666');
                doc.text(`Gerado em: ${generationDate}`, pageWidth - 10, 34, { align: 'right' });

                const pageCount = doc.internal.getNumberOfPages();
                doc.setFontSize(8);
                doc.setTextColor('#999999');
                doc.text(`Página ${data.pageNumber} de ${pageCount}`, pageWidth / 2, doc.internal.pageSize.getHeight() - 10, { align: 'center' }); // Usar doc.internal.pageSize.getHeight() para a altura
            }
        });

        const totalPropostas = reportData.length;

        doc.setFontSize(12);
        doc.setTextColor('#333333');
        const finalY = doc.lastAutoTable?.finalY ?? doc.internal.pageSize.getHeight() - 30; // Usar doc.internal.pageSize.getHeight() para a altura

        doc.text(`Total de Propostas: ${totalPropostas}`, 10, finalY + 15);


        const filename = `relatorio-propostas-${selectedMonth}.pdf`;

        doc.save(filename);
        showToast('Relatório PDF de Propostas gerado com sucesso!', 'success');
    };

    return (
        <Box sx={{ p: 3, display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'flex-start' }}>
            <Typography variant="h5" gutterBottom>
                Gerar Relatório de Propostas de Compra
            </Typography>

            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', width: '100%' }}>
                <FormControl sx={{ minWidth: 200, flexGrow: 1 }}>
                    <TextField
                        type="month"
                        label="Mês e Ano"
                        value={selectedMonth}
                        onChange={(e) => setSelectedMonth(e.target.value)}
                        fullWidth
                        InputLabelProps={{ shrink: true }}
                    />
                </FormControl>
                <Button
                    variant="contained"
                    color="primary"
                    onClick={fetchReportData}
                    sx={{ alignSelf: 'center', minWidth: 150 }}
                >
                    Buscar Dados
                </Button>
            </Box>

            {reportData.length > 0 && (
                <Box sx={{ width: '100%', mt: 3 }}>
                    <Typography variant="h6" gutterBottom>
                        Dados do Relatório:
                    </Typography>
                    <TableContainer component={Paper} sx={{ maxHeight: 400 }}>
                        <Table stickyHeader>
                            <TableHead>
                                <TableRow>
                                    <TableCell align="left">Proponente</TableCell>
                                    <TableCell align="right">ID Imóvel</TableCell>
                                    <TableCell align="right">Valor Imóvel (R$)</TableCell>
                                    <TableCell align="right">Entrada (R$)</TableCell>
                                    <TableCell align="right">Renda Mensal (R$)</TableCell>
                                    <TableCell align="right">Valor Financ. (R$)</TableCell>
                                    <TableCell align="center">Prazo (meses)</TableCell>
                                    <TableCell align="center">Data Proposta</TableCell>
                                    <TableCell align="center">Status</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {reportData.map((row) => (
                                    <TableRow key={row.idProposta}>
                                        <TableCell align="left">{row.nomeProponente}</TableCell>
                                        <TableCell align="right">{row.idImovel}</TableCell>
                                        <TableCell align="right">
                                            {row.valorImovel.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </TableCell>
                                        <TableCell align="right">
                                            {row.valorEntrada.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </TableCell>
                                        <TableCell align="right">
                                            {row.rendaMensal.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </TableCell>
                                        <TableCell align="right">
                                            {row.valorFinanciamento.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </TableCell>
                                        <TableCell align="center">{row.prazo}</TableCell>
                                        <TableCell align="center">
                                            {new Date(row.dataProposta).toLocaleDateString('pt-BR')}
                                        </TableCell>
                                        <TableCell align="center">{row.statusProposta}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
                    </TableContainer>
                    <Button variant="contained" color="secondary" onClick={gerarPDF} sx={{ mt: 2 }}>
                        Gerar PDF
                    </Button>
                </Box>
            )}
        </Box>
    );
};

export default RelatorioPropostas;
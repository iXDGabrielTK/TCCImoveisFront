...
const RelatorioAgendamentos: React.FC = () => {
    ...
    const [reportData, setReportData] = useState<AgendamentoRelatorio[]>([]);
    ...
    const fetchReportData = async () => {
        try {
            ...
            if (filterType === 'year' && selectedYear) { // Usa filterType para decidir qual enviar
                response = await api.get<AgendamentoRelatorio[]>(
                    `/relatorios/agendamentos?ano=${selectedYear}`
                );
            ...
            console.error("Erro ao buscar dados do relatório de agendamentos:", error);
            showToast("Erro ao carregar dados do relatório de agendamentos. Tente novamente.", "error");
            setReportData([]);
        }
    };
    ...
    const gerarPDF = async () => {
        ...
        const colunas = [
            ...
            { header: "Qtd. Agendamentos", dataKey: "quantidadeAgendamentos" },
        ];

        const linhas = reportData.map((item) => ({
            idImovel: item.idImovel?.toString() ?? "N/A",
            descricaoImovel: item.descricaoImovel ?? "N/A",
            quantidadeAgendamentos: item.quantidadeAgendamentos?.toString() ?? "0",
        }));
        ...
        const totalAgendamentos = reportData.reduce(
            (total, item) => total + item.quantidadeAgendamentos,
            0
        );
        ...
        doc.setFontSize(12);
        doc.setTextColor("#333333");
        doc.text(`Total de Agendamentos: ${totalAgendamentos}`, 10, finalY + 15);

        const filename = selectedYear
            ? `relatorio-agendamentos-${selectedYear}.pdf`
            : `relatorio-agendamentos-${selectedMonth}.pdf`;
                        ...
                        <Table stickyHeader>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell align="right">Qtd. Agendamentos</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {reportData.map((row) => (
                                    <TableRow key={row.idImovel}>
                                        ...
                                        <TableCell align="right">{row.quantidadeAgendamentos}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>

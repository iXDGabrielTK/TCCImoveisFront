...
const loadImageAsBase64 = async (url: string): Promise<string> => {
    try {
        ...
        if (!token) {
            ...
            return ''; // Retorna string vazia para que o PDF continue a ser gerado sem a imagem
        }
        ...
        if (!response.ok) {
            // Se o proxy retornar um erro (ex: 403 se o backend não pôde acessar o S3)
            // Adicione mais detalhes ao erro para depuração
            const errorText = await response.text(); // Tente ler a resposta de erro
            throw new Error(`HTTP error from proxy! Status: ${response.status}. Response: ${errorText}`);
        }
        const blob = await response.blob(); // Pega a imagem como um Blob
        ...
        // Retorna uma string vazia para que o PDF continue a ser gerado sem a imagem
        return '';
    }
};
    ...
    const gerarPDF = async () => {
        ...
        autoTable(doc, {
            ...
            didDrawPage: (data: HookData) => {
                ...
                doc.text(`Página ${data.pageNumber} de ${pageCount}`, pageWidth / 2, pageHeight - margin, { align: "center" });
        ...
        yPos += 7;
        doc.text(`Laudo: ${reportData.laudoVistoria}`, margin, yPos);
        yPos += 15;
        ...
        if (reportData.imovel.fotosImovel && reportData.imovel.fotosImovel.length > 0) {
            ...
            for (const foto of reportData.imovel.fotosImovel) {
                ...
                try {
                    // CHAMA O PROXY PARA CARREGAR A IMAGEM COMO BASE64
                    ...
                    // O erro já é capturado dentro de loadImageAsBase64, aqui é só um fallback extra
        ...
        if (reportData.ambientes && reportData.ambientes.length > 0) {
            ...
            for (const ambiente of reportData.ambientes) {
                ...
                yPos += 7;
                doc.text(`  Descrição: ${ambiente.descricao}`, margin + 10, yPos);
                yPos += 10;
                ...
                if (ambiente.fotos && ambiente.fotos.length > 0) {
                    ...
                    let xImg = margin + 10;
                    const imgWidthAmb = 50;
                    const imgHeightAmb = 35;
                    const paddingAmb = 5;

...
import { useToast } from "../context/ToastContext"; // Certifique-se de que o caminho para 'ToastContext' está correto

// Tipagem dos dados do relatório
interface FavoritoRelatorio {
    idImovel: number;
...
const RelatorioFavoritos: React.FC = () => {
    ...
    const [filterType, setFilterType] = useState<'monthYear' | 'year'>('monthYear'); // Novo estado para o tipo de filtro
    const [reportData, setReportData] = useState<FavoritoRelatorio[]>([]);
    ...
    const fetchReportData = async () => {
        try {
            let response;
            // Lógica para determinar qual endpoint chamar (mês/ano ou apenas ano)
            if (filterType === 'year' && selectedYear) { // Usa filterType para decidir qual enviar
                response = await api.get<FavoritoRelatorio[]>(
                    `/relatorios/favoritos?ano=${selectedYear}`
                );
            } else if (filterType === 'monthYear' && selectedMonth) {
                response = await api.get<FavoritoRelatorio[]>(
                    `/relatorios/favoritos?mesAno=${selectedMonth}`
                );
            } else {
                showToast("Por favor, selecione um filtro e um valor para gerar o relatório.", "info");
                return;
            }
            ...
            console.error("Erro ao buscar dados do relatório de favoritos:", error);
            showToast("Erro ao carregar dados do relatório de favoritos. Tente novamente.", "error");
            setReportData([]);
        }
    };
    ...
    const gerarPDF = async () => {
        ...
        let titulo: string;
        if (filterType === 'year' && selectedYear) { // Usa filterType para o título
            titulo = `Relatório Anual de Imóveis Favoritos - ${selectedYear}`;
        } else {
            ...
            titulo = `Relatório Mensal de Imóveis Favoritos - ${mesesPt[parseInt(mes, 10) - 1]} de ${ano}`;
        }
        ...
        const colunas = [
            ...
            { header: "Descrição Imóvel", dataKey: "descricaoImovel" },
            { header: "Qtd. Favoritos", dataKey: "quantidadeFavoritos" },
        ];

        const linhas = reportData.map((item) => ({
            idImovel: item.idImovel?.toString() ?? "N/A",
            descricaoImovel: item.descricaoImovel ?? "N/A",
            quantidadeFavoritos: item.quantidadeFavoritos?.toString() ?? "0",
        }));
        ...
        const totalFavoritos = reportData.reduce(
            (total, item) => total + item.quantidadeFavoritos,
            0
        );
        ...
        doc.setFontSize(12);
        doc.setTextColor("#333333");
        doc.text(`Total de Favoritos: ${totalFavoritos}`, 10, finalY + 15);

        const filename = selectedYear
            ? `relatorio-favoritos-${selectedYear}.pdf`
            : `relatorio-favoritos-${selectedMonth}.pdf`;
    ...
    return (
        <Box sx={{ p: 3, display: "flex", flexDirection: "column", gap: 3, alignItems: "flex-start" }}>
            ...
            <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap", width: "100%" }}>
                {/* Novo Select para escolher o tipo de filtro */}
                <FormControl sx={{ minWidth: 180 }}>
                    ...
                    <Select
                        ...
                        label="Filtrar por"
                        onChange={(e) => {
                            setFilterType(e.target.value as 'monthYear' | 'year');
                            setSelectedMonth(getCurrentMonthYear()); // Reset para o mês atual
                            setSelectedYear(""); // Limpa o ano
                        }}
                    >
                        <MenuItem value="monthYear">Mês e Ano</MenuItem>
                        <MenuItem value="year">Ano</MenuItem>
                    </Select>
                </FormControl>

                {/* Renderização condicional dos campos de entrada */}
                        ...
                        <Table stickyHeader>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell align="left">Descrição Imóvel</TableCell>
                                    <TableCell align="right">Qtd. Favoritos</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {reportData.map((row) => (
                                    <TableRow key={row.idImovel}>
                                        <TableCell align="right">{row.idImovel}</TableCell>
                                        <TableCell align="left">{row.descricaoImovel}</TableCell>
                                        <TableCell align="right">{row.quantidadeFavoritos}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
